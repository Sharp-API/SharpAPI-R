...
sharpapi_get <- function(path, ...) {
  ...
  #
  # Keeping it out of the OBJECT, rather than only out of the printed form,
  # needs httr2 >= 1.2.0: from that version a redacted header is stored behind
  # a weak reference, which does not serialise. httr2 1.0.0-1.1.2 accept
  # `.redact` and mask the header when printing, but still store the plaintext
  # in `req$headers`, so `dput()`, `saveRDS()` or a saved error condition hand
  # the key over. That is why DESCRIPTION floors httr2 at 1.2.0; lowering it
  # silently turns the redaction below into print-time cosmetics.
